...
attach(Results)
head(Results)
####Plotting graphs for woody planats
ggplot(Results) +
  geom_col(aes(x = Site, y = Stem_count), fill = "grey", colour = "black", width = 0.75) +
...
####
ggplot(Results) +
  geom_col(aes(x = Site, y = Mean_height_of_trees), fill = "grey", colour = "black", width = 0.75) +
