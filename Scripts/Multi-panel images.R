...
imgT2 <- image_read("DATA/Treatments/Thinning2.jpeg") 
imgH3 <- image_read("DATA/Treatments/Herbicide1.JPG")
imgH4 <- image_read("DATA/Treatments/Herbicide1.JPG")
...
multi_Tplot <- (pp1 + pp2)/(pp3 + pp4)/(pp5 + pp6)/(pp7+pp8) +
  plot_annotation(tag_levels = 'a', tag_prefix = '(', tag_suffix = ')') &
  theme(plot.tag = element_text(size = 12, face = 'bold', 
                                margin = margin(0, 0, 0, 0, 0, 0, 0, 0))
        
# Save the multi-panel plot
ggsave("Plots/Treatment MPanel_plot.png", multi_plotTT, width = 16, height = 14, units = "cm")
